...
const GridBoard = () => {
  ...
  // Component-scoped unique ID helper
  const COMP_ID = 'c50';
      ...
      <div className="grid grid-cols-8 gap-1 p-6 bg-white rounded-xl shadow-2xl">  
        ...
        {Array.from({ length: 8 }, (_, rowIndex) => (
          <React.Fragment key={`middle-row-${rowIndex}`}>
            ...
            {rowIndex === 0 && (
              <div
                className="col-span-6 row-span-7 grid-cell active:animate-grid-pulse flex items-center justify-center text-white font-bold text-lg relative overflow-hidden"
        ...
        {/* Bottom row - 8 cells */}
        {Array.from({ length: 8 }, (_, colIndex) => {
          const key = cid('bottomA', 9, colIndex);
          return (
            <div
              key={`bottomA-${colIndex}`}
              className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 grid-cell active:animate-grid-pulse relative overflow-hidden"
              style={getCellStyle(key)}
              onClick={() => handleCellClick(key)}
              role="button"
        ...
        {Array.from({ length: 6 }, (_, colIndex) => {
          const key = cid('bottomB', 9, colIndex);
          return (
            <div
              key={`bottomB-${colIndex}`}
              className="w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 grid-cell active:animate-grid-pulse relative overflow-hidden"
              style={getCellStyle(key)}
              onClick={() => handleCellClick(key)}
              role="button"
